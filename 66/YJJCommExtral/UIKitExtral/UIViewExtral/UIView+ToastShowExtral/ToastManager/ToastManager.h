...
///toast设置settings
@property (nonatomic,strong,readonly) ToastSettings *settings;
///显示toast的视图
@property (nonatomic,strong,readonly) UIView *toastView;
///显示toast的父视图,如果为nil将加到window上
@property (nonatomic,weak,nullable) UIView *showInView;
...
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)interval position:(Toast_Position)position image:(nullable UIImage *)image;
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)interval position:(Toast_Position)position title:(nullable NSString *)title;
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)interval position:(Toast_Position)position title:(nullable NSString *)title image:(nullable UIImage *)image;

- (void)makeToast:(NSString *)message duration:(NSTimeInterval)interval centerPoint:(CGPoint)centerPoint;
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)interval centerPoint:(CGPoint)centerPoint image:(nullable UIImage *)image;
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)interval centerPoint:(CGPoint)centerPoint title:(nullable NSString *)title;
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)interval centerPoint:(CGPoint)centerPoint title:(nullable NSString *)title image:(nullable UIImage *)image;
